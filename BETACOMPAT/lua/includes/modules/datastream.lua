...
local function CallStreamHook(pl,h,id,encdat,decdat)
	if not _hooks[h] then
		ErrorNoHalt("DataStreamServer Unknown callback "..tostring(h)..' from '..tostring(pl)..'\n')
	return end
...
local function StreamTick(rep)
	if not rep and iterations > 0 then
	...
	for id,o in pairs(_outgoing) do
		if o.state == 1 then -- Processing stage.
			if o.decdata then
				...
				o.encdata = split(string.gsub(string.gsub(enc,"\\","\\\\"),"\n","\\n"),128)
				o.state = 2
			else
				o.state = 3
			end
